...
def diametre(m_dot, p, h):
    ...
    rho = 1 / H2O.v(p=p, h=h)
    diametre = np.sqrt(m_dot*4/(rho*np.pi*v))

    return diametre, v # metres et m/s
...
def R_conv_nat(Ts, Tinf, dia, L):
    ...
    beta = 1/Tm
    nu = vapeur.kinematic_viscosity
    k = vapeur.conductivity
    rho = vapeur.density
    c = vapeur.specific_heat
    Pr = vapeur.prandtl
    alpha = k / (rho*c)
...
def dQdP(h, h_prop, p, data, BD, dia, v, t):

    mat = BD['Choixm'][0]
    index_mat = BD.index[BD['Matériau'] == mat].tolist()[0]
    k_iso = BD['Conductivité'][index_mat]


    tu = BD['Choixtu'][0]
    index_tu = BD.index[BD['Métal'] == tu].tolist()[0]
    rugosite = BD['Conductivité'][index_tu]
    k_acier = BD['Conductivité'][index_tu]
    ...
    R_tuyau = R_cond(dia, L, t, k_acier)
    R_iso = R_cond(dia + 2 * t, L, t_iso, k_iso)
    R_nat = R_conv_nat(Ts, Tinf, dia + 2 * t + 2 * t_iso, L)
    R_tot = R_c_int + R_tuyau + R_iso + R_nat
