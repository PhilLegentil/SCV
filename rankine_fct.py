...
def pertes_ecoulement(x):
    ...
    Returns
    -------
    pertes : float ou np.array
        pertes par ecoulement dans la turbine HP [kJ/kg]

    '''

    coef_pertes = np.array([5E-8, -7E-5, 0.0371, -8.2873, 678])
    pertes = np.polyval(coef_pertes, x)
    return pertes
...
def travail(m_dot, data):
    ...
    h_4_prime = H2O.h(p=p_4, T=t_2 + dT)
